...
import wiki.scene.tab.delegate.TabCommonDelegate;
import wiki.scene.tab.listener.CustomTabEntity;
import wiki.scene.tab.listener.ITabLayout;
import wiki.scene.tab.listener.OnTabSelectListener;
import wiki.scene.tab.utils.FragmentChangeManager;
import wiki.scene.tab.utils.UnreadMsgUtils;
import wiki.scene.tab.widget.MsgView;
...
public class CommonTabLayout extends FrameLayout implements ValueAnimator.AnimatorUpdateListener, ITabLayout {
    ...
    public void updateTabStyles() {
        for (int i = 0; i < mTabCount; i++) {
            ...
            tabView.setBackgroundResource(i == mCurrentTab ? getDelegate().getTabSelectBg() : getDelegate().getTabDefaultBg());
        }
    }
